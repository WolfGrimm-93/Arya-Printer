...
| CORS `allow_origins=["*"]` + `Access-Control-Allow-Private-Network: true` siempre | Whitelist real (`security.allowed_origins` en `settings.yaml`, vacio por defecto = ningun origen cruzado permitido). `Access-Control-Allow-Private-Network` nunca se emite. |
